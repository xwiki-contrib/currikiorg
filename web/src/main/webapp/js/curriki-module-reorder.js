...
}c.orig=[];a.store=new Ext.data.JsonStore({storeId:"CollectionsStore",url:"/xwiki/curriki/"+c.place+"/"+c.which+"/collections?_dc="+(new Date().getTime()),fields:["displayTitle","collectionPage"],autoLoad:true,listeners:{load:{fn:function(f,e,g){var h=[];
f.each(function(i){h.push(i.data.collectionPage)});c.orig=h;console.log("Fetched list",h)
...
Ext.getCmp("reorderCollectionsMS").store.each(function(h){e.push(h.data.collectionPage)
